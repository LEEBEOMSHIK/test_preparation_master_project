...
| `formatAnswerAlternatives(answer)` | `src/lib/answer.ts` | 정답 문자열의 `\|\|` 대체 정답 구분자(백엔드 `AnswerGrader` — 후보 중 하나만 일치해도 정답)가 있으면 대표 정답(첫 번째 비어있지 않은 항목) 1개만 표시(대체 정답이 많은 문항에서 표시가 지나치게 길어지는 것을 방지, 구분자 없으면 원문 그대로). 사용자 화면 오답 시 "정답:" 표시에만 적용(퀴즈 풀이 `user/quiz/[categoryId]/page.tsx`, `ExamResultDisplay.tsx`) — 관리자 화면(문항 상세/목록)은 원문 그대로 유지, 적용 금지 |
