...
작업 유형에 따라 에이전트를 명확히 분리하여 사용한다.  
분석은 **codebase-explorer**, 설계는 **plan**, 구현은 **webapp-developer**가 담당한다.

### codebase-explorer — 분석 전담

다음 작업은 반드시 `codebase-explorer` 에이전트에 위임한다.
...
**규칙**
- codebase-explorer는 파일을 읽고 탐색하되, 코드를 직접 수정하지 않는다.
- 탐색 결과를 요약해 plan 또는 webapp-developer에 전달한다.
- 탐색 범위가 3개 쿼리 이상 필요한 개방형 탐색은 반드시 이 에이전트를 사용한다.
...
| 조건 | 이유 |
|------|------|
| 여러 파일에 걸친 수정이 필요한 경우 | 파일 간 의존 관계·수정 순서가 설계 단계에서 결정되어야 함 |
| 기존 시스템과의 연결 고리가 불명확한 경우 | 잘못된 연결 방식으로 구현하면 수정 비용이 큼 |
| 여러 설계 옵션이 존재해 트레이드오프 판단이 필요한 경우 | 구현 전에 방향을 확정해야 낭비 없이 진행 가능 |

**plan을 생략하고 webapp-developer 직행하는 경우**

| 조건 | 이유 |
|------|------|
| 신규 파일 1개만 추가하는 단순 작업 | 설계 범위가 자명하여 plan 단계가 불필요 |
| codebase-explorer 분석으로 설계가 이미 명확한 경우 | 추가 설계 없이 즉시 구현 착수 가능 |

**규칙**
- plan은 수정 대상 파일 목록, 변경 순서, 설계 결정 이유를 명확히 문서화한다.
- plan 결과를 webapp-developer에 전달할 때는 파일 경로·메서드명·인터페이스 변경 사항을 구체적으로 포함한다.
- 코드 작성·파일 수정은 절대 하지 않는다.

---

### webapp-developer — 구현 전담

다음 작업은 반드시 `webapp-developer` 에이전트에 위임한다.
...
**규칙**
- plan 또는 codebase-explorer 결과를 전달받아 코드를 작성·수정한다.
- TPMP 프로젝트의 컨벤션(TypeScript strict, Tailwind, Controller-Service-Repository 3레이어)을 반드시 준수한다.
- 구현 완료 후 수정 파일과 검증 결과를 정리하고, **Modification History Policy**에 따라 히스토리 파일을 자동 작성한다.
...
[2] plan 사용 여부 판단
    ...
    │       ▼
    │   [3] plan — 설계 수립
    │       • 수정 대상 파일 목록 및 변경 순서 결정
    │       • 설계 옵션 비교 및 트레이드오프 판단
    │       • 구체적 인터페이스·메서드 시그니처 확정
    │       ▼
    │   [4] webapp-developer — 코드 구현
    │       • plan 결과를 기반으로 코드 작성·수정
    │       ▼
    │   [5] webapp-developer — 정리 및 검증
    │       • 수정 파일 목록 정리 및 타입 오류·컨벤션 확인
    │       • 히스토리 파일 작성, CLAUDE.md 관련 섹션 업데이트
    │
    └─ [plan 생략] ──────────────────────────────────────────────
            ▼
        [3] webapp-developer — 코드 구현 (직행)
            • codebase-explorer 결과만으로 즉시 구현
            ▼
        [4] webapp-developer — 정리 및 검증
            • 수정 파일 목록 정리 및 타입 오류·컨벤션 확인
            • 히스토리 파일 작성, CLAUDE.md 관련 섹션 업데이트
```
...
| 조건 | plan 사용 여부 |
|------|--------------|
| 수정 파일이 3개 이상이고 파일 간 의존 관계가 있는 경우 | 사용 |
| 기존 코드 연결 방식이 분석 결과만으로 불명확한 경우 | 사용 |
| 구현 방식의 선택지가 2개 이상 존재하는 경우 | 사용 |
| 신규 파일 1개 추가 또는 단순 필드 추가 | 생략 |
| codebase-explorer 결과로 구현 경로가 명확한 경우 | 생략 |

### AI 행동 규칙

1. codebase-explorer는 항상 먼저 실행하고 결과를 수집한 뒤 다음 단계를 결정한다.
2. plan 단계가 필요한지 여부는 위 판단 기준표에 따라 결정하고, 판단 이유를 사용자에게 한 줄로 설명한다.
3. 단순 파일 읽기(경로가 이미 알려진 경우)는 에이전트 없이 `Read`·`Grep` 도구를 직접 사용한다.
4. webapp-developer가 구현 중 추가 탐색이 필요하면, 탐색 쿼리가 3개 미만인 경우 직접 처리하고, 이상이면 codebase-explorer에 위임한다.
5. 작업 결과가 CLAUDE.md에 영향을 주는 경우(새 유틸 추가 등) webapp-developer가 **즉시 이 파일을 업데이트**한다.
