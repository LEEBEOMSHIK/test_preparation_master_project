...
분석과 탐색은 **codebase-explorer**, 코드 작성·수정은 **webapp-developer**가 담당한다.
...
- 탐색 결과를 요약해 webapp-developer 또는 메인 에이전트에 전달한다.
...
**규칙**
- webapp-developer는 탐색 결과를 전달받아 코드를 작성·수정한다.
- TPMP 프로젝트의 컨벤션(TypeScript strict, Tailwind, Controller-Service-Repository 3레이어)을 반드시 준수한다.
- 코드 수정 완료 후 **Modification History Policy**에 따라 히스토리 파일을 자동 작성한다.

### 역할 분담 흐름 (표준 패턴)

두 에이전트는 기본적으로 **병렬 실행**한다.  
탐색 결과가 구현의 선행 조건일 때만 순차 처리한다.

```
사용자 요청
    │
    ├─ [병렬 실행 — 탐색과 구현 범위가 독립적인 경우]
    │       codebase-explorer ──────────────────────┐
    │       • 관련 파일·패턴·아키텍처 탐색           │ 동시 실행
    │                                               │
    │       webapp-developer ─────────────────────┘
    │       • 이미 알려진 범위 내에서 코드 작성·수정
    │       • 수정 완료 후 히스토리 파일 작성
    │       • CLAUDE.md 관련 섹션 즉시 업데이트
    │
    └─ [순차 실행 — 탐색 결과가 구현의 전제 조건인 경우]
            codebase-explorer → (결과 전달) → webapp-developer
```
...
| 조건 | 실행 방식 |
|------|----------|
| 구현할 파일·API·컴포넌트 범위가 이미 명확한 경우 | 병렬 |
| 탐색 결과가 없어도 구현 착수 가능한 경우 | 병렬 |
| 탐색 결과에 따라 구현 전략이 달라지는 경우 | 순차 |
| 기존 코드를 반드시 파악한 후에야 수정 가능한 경우 | 순차 |

### AI 행동 규칙

1. 탐색과 구현이 모두 필요한 작업은 **기본적으로 병렬 실행**한다. 탐색 결과가 구현의 전제 조건일 때만 순차 처리한다.
2. 단순 파일 읽기(경로가 이미 알려진 경우)는 에이전트 없이 `Read`·`Grep` 도구를 직접 사용한다.
3. webapp-developer가 구현 중 추가 탐색이 필요하면, 탐색 쿼리가 3개 미만인 경우 직접 처리하고, 이상이면 codebase-explorer에 위임한다.
4. 두 에이전트의 작업 결과가 CLAUDE.md에 영향을 주는 경우(새 유틸 추가 등) **즉시 이 파일을 업데이트**한다.
5. 병렬 실행 시 두 에이전트의 결과를 취합한 뒤 충돌(파일 중복 수정 등)이 있으면 webapp-developer 결과를 기준으로 병합한다.
