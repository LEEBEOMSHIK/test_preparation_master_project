...
| `<InterestExamTypeModal open onClose onSaved />` | `src/components/ui/InterestExamTypeModal.tsx` | 관심 시험 유형 설정 모달(`examInfoService.getExamTypes`/`updateInterests` 호출, 저장 성공 시 authStore.user 즉시 갱신) — `user/exam-info`·`user/settings` 공용, `onSaved`는 저장 성공 후(모달이 닫히기 직전) 호출되어 호출부 후처리(목록 재조회 등)에 사용 |
...
| `<BugReportModal context={{source, label, questionId?, questionContent?}} onClose />` | `src/components/ui/BugReportModal.tsx` | 퀴즈·시험 풀이·개념노트 상세 화면 문항(또는 노트) 단위 축약 버그 신고 모달 — 컨텍스트 자동 첨부 후 설명만 입력하면 1:1 문의(BUG 유형)로 즉시 등록. `source`는 `'QUIZ'\|'EXAM'\|'CONCEPT_NOTE'` |
...
| `<SqlProblemView data className />` | `src/components/ui/SqlProblemView.tsx` | SQL 구조화 문항(SQL 유형) 표시용 뷰 — 표/스키마(DDL) 토글, PK 배지. `data.expectedResult`(결과 테이블 정답)가 있으면 모드와 무관하게 하단에 "결과 테이블 정답" 표(orderedRows면 "순서 채점" 배지)를 항상 함께 표시 — 관리자 응답에만 포함되고 퀴즈 노출 DTO는 항상 제거되므로 자연히 관리자 화면에서만 노출됨 |
| `<SqlProblemEditor value onChange />` | `src/components/ui/SqlProblemEditor.tsx` | SQL 문항 등록/수정 에디터 — 테이블 카드 반복(테이블명·컬럼 편집·샘플 데이터 행 추가/삭제) + "결과 테이블 정답 (선택)" 섹션("+ 결과 테이블 정답 추가"로 활성화, 컬럼명·정답 행 그리드, "행 순서까지 채점(ORDER BY 문제)" 체크박스, 셀에 `\|` 입력 시 경고 문구, "섹션 제거") |
| `emptySqlDraft`/`toSqlDataPayload`/`fromSqlData`/`isExpectedResultEnabled`/`serializeSqlResult` 등 | `src/lib/sql.ts` | SQL 문항 등록/수정 폼 공용 헬퍼 (Draft ↔ SqlData 변환). `SqlDataDraft`에 `expectedResult: SqlExpectedResultDraft` 포함, `isExpectedResultEnabled`는 이름 있는 컬럼이 1개 이상이면 true(활성 판정), `serializeSqlResult(expectedResult)`는 `"c1 \| c2\nv1 \| v2"` 형식 직렬화 — 등록/수정 페이지에서 expectedResult 활성 시 answer 필드 자동 세팅용 |
| `<SqlResultAnswerInput columns value onChange disabled />` | `src/components/ui/SqlResultAnswerInput.tsx` | SQL "결과 테이블(컬럼×튜플)" 정답 입력 그리드(퀴즈 풀이 전용) — `question.sqlResultColumns`(컬럼명만, 정답 유출 방지로 expectedResult 자체는 절대 미노출)가 있을 때 CodeAnswerInput 대신 사용. 열=컬럼명 고정, "+ 행 추가"/행 삭제, 제출 시 `셀 \| 셀` + 줄바꿈 직렬화(기존 채점 API 문자열 계약 유지). 호출부가 `key={question.id}`로 문항 전환 시 리마운트해 그리드를 초기화해야 함 |
...
| `<ThemeToggle className />` | `src/components/ui/ThemeToggle.tsx` | 다크모드 토글 버튼(해/달 아이콘) — 레이아웃 셸 헤더·사용자/관리자 로그인 화면 공용, `className`으로 크기·색 재정의 가능 |
...
| `parseTraceLines(text)`, `type TraceLine` | `src/lib/traceNotation.ts` | 코드 트레이싱 표기법(`name = value`/`name: type = value`/`name = [a,b]`/`name = [[1,2],[3,4]]`/`name = [1,2][3,4]`(나란히 2D)/`name = {k: v, ...}`(오브젝트)) 순수 파서 — eval/Function/JSON.parse 미사용, 실패 시 text로 안전 폴백. 값 기반 타입 자동추론 + `: type` 명시 오버라이드(typeLabel/typeSource). 2D 배열은 중첩 `[[..],[..]]`뿐 아니라 대괄호 그룹 2개 이상이 공백만 사이에 두고 이어져 전체 rhs를 덮는 **나란히 표기**도 인식(각 그룹 파싱 실패 시 전체 text 폴백). 오브젝트(`{`로 시작)는 최상위 콤마로 엔트리 분리 후 각 엔트리를 첫 최상위 콜론으로 key/value 분리해 `ObjectLine`(typeLabel 기본 `object`)으로 렌더 — 콜론 없는 엔트리가 하나라도 있으면(Set 형태 등) 전체 text 폴백, 값은 재귀 파싱하지 않고 문자열 그대로. **변수 참조 산술·비트 수식 자동 계산**(`evaluateExpression` 재사용)을 픽스포인트 다중 패스로 지원 — (1) 식별자 없는 순수 리터럴 수식도 계산(`av = 10 / 4` → 2.5, `mask = 0b1010 & 3` → 2, 단 날짜·버전·전화번호형 압축 나열(`2024-01-01` 등)은 가드로 제외), (2) 변수 정의가 참조보다 아래 줄에 있어도(순서 무관) 계산. 같은 이름 재대입 시 텍스트상 마지막 리터럴이 최종값. 미정의 참조·순환·계산 오류는 문자열로 안전 폴백, 결과는 `VarLine.sourceExpr`+`value`. **`=` 없이 수식만 적은 줄도 계산**(예: `av / len`, `mask << 1` → `ExprLine`, 이름 없음·env 미등록) — 산술·비트 연산자 최소 1개 포함 + 화이트리스트 통과 + 참조 식별자가 named 변수 픽스포인트 종료 후 최종 env에 전부 존재해야 계산, 그 외(단어 하나·단일 식별자·자유 문장·미정의 참조)는 text로 폴백. 레거시 traceBlocks 이관용 `sanitizeLegacyTraceBlocks`·`legacyTraceBlocksToNotation`도 포함 |
| `<TracePreview lines />` | `src/components/ui/TracePreview.tsx` | traceNotation 파싱 결과(변수 행·1D 배열·2D 배열(중첩·나란히 공용 렌더)·오브젝트·자유 텍스트·이름 없는 수식)를 실시간 렌더 + 타입 배지(explicit/inferred 시각 구분) + 자동 계산된 변수는 `이름 = 수식 = 결과` 형태로 원본 수식도 함께 표시하는 읽기 전용 프리뷰. 이름 없는 수식 줄(`kind: 'expr'`)은 `수식 = 결과` + 타입 배지만 표시(이름 칩 없음). 오브젝트 줄(`kind: 'object'`)은 `ObjectRow`가 이름+타입 배지 아래 키-값 2열 표로 렌더(빈 객체는 "(빈 객체)") |
| `<CodeLanguageModal open onClose showLanguage showSource onSelect />` | `src/components/ui/CodeLanguageModal.tsx` | 데일리 퀴즈에서 CODE(프로그래밍 언어) 카테고리·AI 커스텀 문항 보유 카테고리 선택 시 언어(Java/Python/C/전체)·출처(기출/AI 커스텀/전체)를 고르는 모달(AlertModal 패턴 준용, ESC·오버레이 클릭 닫기). `showLanguage`/`showSource`로 섹션 표시 여부를 개별 제어하며, 둘 다 true면 두 섹션을 함께 보여주고 "시작" 버튼으로 확정한다. `onSelect({ language?, source? })` — 미표시 섹션의 값은 `undefined` |
...
| `formatAnswerAlternatives(answer, disableAlternative?)` | `src/lib/answer.ts` | 정답 문자열의 `\|\|` 대체 정답 구분자(백엔드 `AnswerGrader` — 후보 중 하나만 일치해도 정답)가 있으면 대표 정답(첫 번째 비어있지 않은 항목) 1개만 표시(대체 정답이 많은 문항에서 표시가 지나치게 길어지는 것을 방지, 구분자 없으면 원문 그대로). 2번째 인자 `disableAlternative`(문항의 `disableAlternativeAnswer` 플래그)가 true면 `\|\|`를 대체 정답 구분자로 해석하지 않고 원문 그대로 반환(코드 조건의 논리 OR `a < m \|\| b[a] < x` 보호용, 인자 생략 시 기존 동작과 동일). 사용자 화면 오답 시 "정답:" 표시에만 적용(퀴즈 풀이 `user/quiz/[categoryId]/page.tsx`, `ExamResultDisplay.tsx`) — 관리자 화면(문항 상세/목록)은 원문 그대로 유지, 적용 금지 |
