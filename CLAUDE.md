...
| `formatAnswerAlternatives(answer)` | `src/lib/answer.ts` | 정답 문자열의 `\|\|` 대체 정답 구분자(백엔드 `AnswerGrader` — 후보 중 하나만 일치해도 정답)를 사용자 노출용 `"A 또는 B"` 형태로 join(구분자 없으면 원문 그대로). 사용자 화면 오답 시 "정답:" 표시에만 적용(퀴즈 풀이 `user/quiz/[categoryId]/page.tsx`, `ExamResultDisplay.tsx`) — 관리자 화면(문항 상세/목록)은 원문 그대로 유지, 적용 금지 |
