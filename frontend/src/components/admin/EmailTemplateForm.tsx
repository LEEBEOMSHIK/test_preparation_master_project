...
type EmailTemplateOperation = 'save' | 'preview' | 'testSend';

export function EmailTemplateForm({ mode, templateId }: EmailTemplateFormProps) {
  const editorRef = useRef<RichTextEditorHandle>(null);
  const inputRevision = useRef(0);
  const operationRef = useRef<EmailTemplateOperation | null>(null);
  ...
  const [operation, setOperation] = useState<EmailTemplateOperation | null>(null);
  const [error, setError] = useState('');
  const [message, setMessage] = useState('');
  const saving = operation === 'save';
  const previewing = operation === 'preview';
  const sending = operation === 'testSend';

  const beginOperation = (nextOperation: EmailTemplateOperation): boolean => {
  ...
  const finishOperation = (completedOperation: EmailTemplateOperation): void => {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      const savedRevision = inputRevision.current + 1;
      inputRevision.current = savedRevision;
      ...
      if (preview) {
        try {
          ...
          if (inputRevision.current === savedRevision) {
    ...
    } finally { finishOperation('save'); }
  };

  const showPreview = async () => {
    if (!beginOperation('preview')) return;
    setError(''); setMessage('');
    const requestRevision = inputRevision.current;
    const requestSubject = subjectTemplate.trim();
    const requestHtmlBody = htmlBody;
    try {
      const response = await emailTemplateService.preview({ scope: 'INQUIRY_STATUS', subjectTemplate: requestSubject, htmlBody: requestHtmlBody });
      if (!response.data.success || !response.data.data) throw new ApiApplicationError(response.data.error?.message ?? '미리보기에 실패했습니다.');
      if (inputRevision.current === requestRevision) setPreview(response.data.data);
    } catch (requestError: unknown) {
      if (inputRevision.current === requestRevision) {
        setError(extractApiErrorMessage(requestError, '이메일 미리보기에 실패했습니다.'));
      }
    } finally { finishOperation('preview'); }
  };

  const testSend = async () => {
    if (mode !== 'edit' || !templateId) return;
    if (!beginOperation('testSend')) return;
    setError(''); setMessage('');
    try {
    ...
    } finally { finishOperation('testSend'); }
  };
  ...
  const updateSubjectTemplate = (value: string) => {
    inputRevision.current += 1;
    setSubjectTemplate(value);
    setPreview(null);
  };

  const updateHtmlBody = (value: string) => {
    inputRevision.current += 1;
    setHtmlBody(value);
    setPreview(null);
  };
  ...
  return (
    <form onSubmit={save} className="space-y-6">
      ...
      <div className="grid gap-4 rounded-xl border border-gray-200 bg-white p-5 sm:grid-cols-2">
        <label className="space-y-1 text-sm font-medium text-gray-700"><span>템플릿 이름</span><input aria-label="템플릿 이름" maxLength={100} value={name} disabled={saving} onChange={(event) => setName(event.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 font-normal" /></label>
        ...
        <label className="space-y-1 text-sm font-medium text-gray-700 sm:col-span-2"><span>제목 템플릿</span><input aria-label="제목 템플릿" maxLength={200} value={subjectTemplate} disabled={saving} onChange={(event) => updateSubjectTemplate(event.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 font-normal" /></label>
        <label className="flex items-center gap-2 text-sm font-medium text-gray-700 sm:col-span-2"><input type="checkbox" checked={active} disabled={saving} onChange={(event) => setActive(event.target.checked)} />활성 템플릿</label>
      ...
      <section className="space-y-3 rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div className="flex flex-wrap gap-2">{allowedVariables.map((variable) => <button key={variable.name} type="button" title={variable.description} aria-label={`${variable.label} 삽입`} disabled={saving} onClick={() => editorRef.current?.insertText(variable.token)} className="rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-medium text-indigo-700 disabled:opacity-50">{variable.label} <code>{variable.token}</code></button>)}</div>
        <div><label id="email-template-html-body-label" className="mb-1 block text-sm font-medium text-gray-700">HTML 본문</label><RichTextEditor ref={editorRef} value={htmlBody} onChange={updateHtmlBody} minHeight={300} allowImages={false} disabled={saving} ariaLabelledBy="email-template-html-body-label" /></div>
      </section>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-2"><button type="submit" disabled={operation !== null} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? '저장 중...' : '저장'}</button><button type="button" disabled={operation !== null} onClick={() => void showPreview()} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium disabled:opacity-50">{previewing ? '미리보기 생성 중...' : '미리보기'}</button><Link href="/admin/email-templates" className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium">목록</Link></div>
        {mode === 'edit' && <div className="flex items-center gap-3 rounded-lg bg-gray-50 px-3 py-2 text-sm"><span className="text-gray-500">테스트 수신자</span><strong>{adminEmail || '관리자 이메일 확인 불가'}</strong><button type="button" disabled={operation !== null || !templateId || !adminEmail} onClick={() => void testSend()} className="rounded bg-gray-800 px-3 py-1.5 text-white disabled:opacity-40">{sending ? '발송 중...' : '테스트 발송'}</button></div>}
      </div>
