    ...
    <div className="mt-2 space-y-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button type="button" onClick={handleRegenerate}
          ...
          title={!result ? '키워드 추출 후 사용 가능합니다' : undefined}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition
        ...
        <button type="button" onClick={() => setGenOpen(o => !o)}
          className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition ${
            genOpen
              ? 'border-blue-400 text-blue-700 bg-blue-100 dark:border-blue-500 dark:text-blue-200 dark:bg-blue-900/50'
              : 'border-blue-200 text-blue-600 bg-blue-50 hover:bg-blue-100 dark:border-blue-700 dark:text-blue-400 dark:bg-blue-900/20 dark:hover:bg-blue-900/40'
          }`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/>
          </svg>
          {genOpen ? 'AI 생성 닫기' : 'AI 문제 생성'}
        </button>
      </div>
      ...
      {/* ── 키워드 분석 결과 패널 (violet) ── */}
      {result && !analyzing && (
        <div className="relative rounded-xl border border-violet-200 bg-violet-50 dark:border-violet-700 dark:bg-violet-950/60 p-4 space-y-3">
      ...
      {genOpen && (
        <div className="rounded-xl border border-blue-200 bg-blue-50 dark:border-blue-700 dark:bg-blue-950/60 p-4 space-y-3">
          <p className="text-xs font-semibold text-blue-700 dark:text-blue-300">AI 문제 생성</p>
      ...
      {regenerated && !regenerating && (
        <div className="rounded-xl border border-indigo-200 bg-indigo-50 dark:border-indigo-700 dark:bg-indigo-950/60 p-4 space-y-2">
          ...
          {onApplyContent && (
            <button type="button" onClick={() => { if (regenerated && onApplyContent) { onApplyContent(regenerated); setRegenerated(null); } }}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition
