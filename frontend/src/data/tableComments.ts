...
export const TABLE_COMMENTS: TableComment[] = [
  ...
  },
  {
    ...
    tableComment: '이메일 템플릿',
    columns: {
      id: 'PK',
      name: '관리자 표시용 템플릿 이름',
      scope: '템플릿 적용 범위 (INQUIRY_STATUS)',
      subject_template: '변수 치환 전 메일 제목 템플릿',
      html_body: '변수 치환 전 HTML 메일 본문 템플릿',
      text_body: '변수 치환 전 일반 텍스트 메일 본문 템플릿',
      active: '템플릿 활성 여부',
      system_key: '기본 시스템 템플릿 식별자, nullable, unique',
      created_by_admin_id: 'FK → users.id (등록 관리자, nullable)',
      updated_by_admin_id: 'FK → users.id (최종 수정 관리자, nullable)',
      created_at: '생성 일시',
      updated_at: '최종 수정 일시',
      deleted_at: '소프트 삭제 일시, nullable',
      deleted_by_admin_id: 'FK → users.id (삭제 관리자, nullable)',
    },
    fkRelations: [
      { column: 'created_by_admin_id', foreignTable: 'users', foreignColumn: 'id', displayColumn: 'name' },
      { column: 'updated_by_admin_id', foreignTable: 'users', foreignColumn: 'id', displayColumn: 'name' },
      { column: 'deleted_by_admin_id', foreignTable: 'users', foreignColumn: 'id', displayColumn: 'name' },
    ],
  },
  {
    table: 'email_template_bindings',
    tableComment: '이메일 이벤트별 템플릿 연결',
    columns: {
      event_code: 'PK, 문의 상태 이메일 이벤트 코드',
      template_id: 'FK → email_templates.id (ON DELETE RESTRICT)',
      created_by_admin_id: 'FK → users.id (연결 관리자, nullable)',
      updated_by_admin_id: 'FK → users.id (최종 변경 관리자, nullable)',
      created_at: '생성 일시',
      updated_at: '최종 수정 일시',
    },
    fkRelations: [
      { column: 'template_id', foreignTable: 'email_templates', foreignColumn: 'id', displayColumn: 'name' },
      { column: 'created_by_admin_id', foreignTable: 'users', foreignColumn: 'id', displayColumn: 'name' },
      { column: 'updated_by_admin_id', foreignTable: 'users', foreignColumn: 'id', displayColumn: 'name' },
    ],
  },
  {
    ...
    columns: {
      ...
      subject: '메일 제목',
      body: '메일 본문',
      html_body: '발송 시점의 정화된 HTML 메일 본문 스냅샷, 일반 텍스트 메일은 nullable',
