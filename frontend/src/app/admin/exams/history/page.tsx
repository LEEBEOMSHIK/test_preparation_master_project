...
export default function ExamHistoryPage() {
  ...
  // 마지막 컬럼(응시 일시)은 읽기전용이라 폭은 그대로 두고 localStorage 키만 v2로 갱신
  const { widths, startResize } = useColumnResize('tpmp:admin-exam-history:col-widths:v2', [56, 120, 180, 240, 80, 80, 80, 160]);
          ...
          <table className="w-full text-sm table-fixed">
            ...
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider relative">시험명<ColResizeHandle onMouseDown={(e) => startResize(3, e)} /></th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider relative">총 문항<ColResizeHandle onMouseDown={(e) => startResize(4, e)} /></th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider relative">정답 수<ColResizeHandle onMouseDown={(e) => startResize(5, e)} /></th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider relative">점수<ColResizeHandle onMouseDown={(e) => startResize(6, e)} /></th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap relative">응시 일시</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-700 dark:text-gray-300 overflow-hidden truncate" title={item.examinationTitle}>
