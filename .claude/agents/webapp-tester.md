...
변경 범위에 맞는 가장 작은 검증부터 실행한다. 전체 빌드·전체 테스트는 마지막 1회 확인을 기본으로 하며, 장황한 성공 로그와 기존 경고는 요약만 보고한다.
