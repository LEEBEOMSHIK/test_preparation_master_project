...
- **날짜**: 2026-07-22
- **수정 범위**: 사용자 백엔드 / 시험 데이터 (exams, examinations) — AI 커스텀 시험 6개 "TPMP 모의고사 N회" 번호 순서 정정
- **수정 개요**: HIST-20260722-002(`20260722_04` 마이그레이션)에서 적용한 회차 번호가 반대로 매겨져 있었다(최신 시험이 1회, 가장 오래된 시험이 6회). 사용자 정정에 따라 올바른 규칙(가장 오래된 시험=1회, 가장 최신 시험=가장 큰 번호)으로 `exams.title`/`examinations.title`을 다시 UPDATE했다. `exam_year`/`exam_round`/`is_ai_custom` 구조화 컬럼은 변경 없음.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| docs/db-migration/20260722_05_fix_ai_custom_exam_title_order.sql | 추가 | exams·examinations 각 6행 title UPDATE(오래된순 1회→최신순 6회로 정정), 적용 전/후 검증 SELECT 포함, 롤백 문 포함 |

### 수정 상세

#### `docs/db-migration/20260722_05_fix_ai_custom_exam_title_order.sql`
- 변경 전(HIST-20260722-002 적용 직후, 반대 순서 — exams.id/examinations.id/title):
  - 13/21: TPMP 모의고사 1회 (실제로는 2026-2, 최신)
  - 8/16: TPMP 모의고사 2회 (2026-1)
  - 9/17: TPMP 모의고사 3회 (2025-3)
  - 10/18: TPMP 모의고사 4회 (2025-2)
  - 11/19: TPMP 모의고사 5회 (2025-1)
  - 12/20: TPMP 모의고사 6회 (실제로는 2024-3, 가장 오래됨)
- 변경 후(로컬 tpmp-db에 실제 적용 완료, 검증 SELECT로 확인):
  - 12/20 (2024-3, 가장 오래됨) → TPMP 모의고사 1회
  - 11/19 (2025-1) → TPMP 모의고사 2회
  - 10/18 (2025-2) → TPMP 모의고사 3회
  - 9/17 (2025-3) → TPMP 모의고사 4회
  - 8/16 (2026-1) → TPMP 모의고사 5회
  - 13/21 (2026-2, 최신) → TPMP 모의고사 6회
- 이유: 사용자가 "가장 최신 시험이 마지막 회차가 되어야 하는데 거꾸로 되어 있다"고 정정. 오래된 시험일수록 낮은 회차 번호, 최신 시험일수록 높은 회차 번호가 되어야 자연스러운 회차 흐름과 일치함
- 프론트엔드 확인: `frontend/src/app/user/exams/page.tsx`는 `exam.title`을 DB 값 그대로 렌더링하며 회차 번호를 프론트에서 생성하는 로직이 없음을 확인(하드코딩 없음, 추가 수정 불필요). `/user/exams` 화면에서 브라우저로 6회=2026-2(최신)·1회=2024-3(가장 오래됨) 매핑을 육안 확인함

### 복원 방법
이 ID(HIST-20260722-003)만으로 복원 시 `docs/db-migration/20260722_05_fix_ai_custom_exam_title_order.sql` 하단 ROLLBACK 블록(주석 처리됨)을 별도 트랜잭션으로 실행해 HIST-20260722-002가 적용한 반대 순서 title 6개(exams·examinations 각각)로 원복한다.
...
- **날짜**: 2026-07-21
- **수정 범위**: 콘텐츠 데이터 / 2026년 2회 원본(기존 그대로) 20문항을 문제은행(question_bank)에 등록
- **수정 개요**: 정식 기출 회차(2026-1 등)와 동일하게 exam_year=2026, exam_round=2로 `question_bank`에 20문항을 등록했다(AI 커스텀이 아닌 정식 기출 취급, ai_* 메타데이터 없음). exam questions에서 content·code·answer·options·instruction·explanation을 그대로 복사했다.
- **파일**: `docs/db-migration/20260721_08_original_questionbank_2026_r2.sql` (신규, 동일 title 존재 시 미삽입으로 재실행 안전). 로컬 적용.
- **검증**: 20행 삽입 확인.

## HIST-20260721-007

- **날짜**: 2026-07-21
- **수정 범위**: 콘텐츠 데이터 / "2026년 2회 정보처리기사 실기" 원본(기존 그대로) 시험 신규 생성
- **수정 개요**: 사용자가 직접 원문 텍스트를 붙여넣고 표/코드 스크린샷 8장(`references/images/img.png`~`img_7.png`)을 제공하여, 외부 블로그 스크래핑이 아닌 **사용자 제공 자료**를 근거로 20문항 전체를 텍스트/HTML 표로 등록했다(exams + questions 20 + examinations). 이미지로만 표현해야 하는 문항은 없었다. 정답은 추측하지 않고 CODE는 실제 실행(Java 2·Python 2)·시뮬레이션(C 3), SQL 2문항은 sqlite3 실제 실행(13번 상관 서브쿼리=3, 14번 RIGHT OUTER JOIN=2), 계산형(8번 SRT 평균 대기시간=6.5ms, 16번 프리픽스=28)은 직접 계산으로 검증했다.
- **10번(서브네팅) 설계**: 원본은 "대역 안 임의 호스트 주소"를 채우는 자유 입력 형식이라 정답이 여러 개 가능해 현재 채점 로직(정확 일치)과 맞지 않았다. 사용자 지시로 **보기(ㄱ~ㅂ) 중 유효한 것을 고르는 형식**으로 설계 변경 — 정답 3개(ㄱ·ㄷ·ㅁ)는 Python `ipaddress` 모듈로 서브넷 소속 여부를 검증했다. 최초 시도에서 `options` 필드에 IP 텍스트를 넣고 정답은 "ㄱ/ㄷ/ㅁ" 라벨로 써서 채점이 실패하는 버그가 있었고(옵션 배열과 정답 라벨 불일치), `options`를 제거하고 순수 텍스트 다중값 비교(기존 회차에서 검증된 안전한 패턴)로 수정했다.
- **20번(정규형) 설계**: 표 데이터만으로 함수적 종속이 특정되지 않아 불확실했으나, 사용자가 "표만 보고 풀이 가능한 문제가 맞다"고 확인해 원문 표·정답("제 3정규형", 사용자가 붙여넣은 원문에 "더보기" 토글이 열려 노출되어 있던 정답)을 그대로 반영했다.
- **파일**: `docs/db-migration/20260721_07_original_exam_2026_r2.sql` (신규, 재실행 안전). 로컬 tpmp-db 적용.
- **검증**: user 계정 응시(start→submit) — 20/20·100점 확인 후 테스트 이력 삭제.
