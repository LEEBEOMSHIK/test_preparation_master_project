...
- **날짜**: 2026-07-08
- **수정 범위**: 관리자 프론트엔드 / 회원 관리 — 테이블 클리핑 버그 수정
...
| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/admin/users/page.tsx` | 수정 | 목록 `<table>`을 `<div className="overflow-x-auto">`로 감쌈 |
...
- **날짜**: 2026-04-30
- **수정 범위**: 관리자 프론트엔드 / 계정 관리
- **수정 개요**: 계정 목록에 이름/이메일 키워드 검색 조건 추가

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/admin/users/page.tsx` | 수정 | `keyword`/`appliedKeyword` 상태 추가, `filteredUsers` useMemo, 검색 UI 패널 추가 |

### 수정 상세

- **변경 전**: 탭(전체/사용자/관리자)으로만 필터링, 키워드 검색 없음
- **변경 후**: 키워드 입력 → 검색 버튼 클릭 시 이름·이메일 기준 클라이언트 사이드 필터링
- **이유**: 계정이 많을 경우 특정 사용자를 빠르게 찾을 수 있도록

### 복원 방법

이 ID(HIST-20260430-004)로 복원 시: `keyword`, `appliedKeyword`, `handleSearch`, `handleReset`, `filteredUsers` 제거, 검색 UI 패널 제거, `filteredUsers.map` → `users.map` 복원
