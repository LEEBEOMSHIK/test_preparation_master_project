...
- **날짜**: 2026-08-03
- **수정 범위**: 관리자 프론트엔드 / 문항 관리 목록(`admin/exams/questions`) — 시험 유형·사용 시험 필터·컬럼 추가, 전체 문항 로드 누락 수정
- **수정 개요**: 백엔드가 이미 내려주던 `examTypeName`(시험 유형)이 목록 화면에서 전혀 노출되지 않았고, 문항이 실제로 어느 시험지에 쓰이는지 확인할 방법도 없었다(사용자 리포트). "시험 유형" 필터·컬럼과, 백엔드가 새로 내려주는 `usedInExams`(사용 시험) 기반 "사용 시험"/"사용 여부" 필터·컬럼(시험명 배지)을 추가했다. 검증 중 `총 문항 수(629) > 고정 조회 size(500)`라 뒤 페이지 129건이 목록에 아예 안 잡히던 기존 버그(오늘 문항이 늘며 처음 500을 넘겨 드러남)를 발견해 함께 고쳤다 — `totalPages`만큼 순회해 전체를 모으도록 변경.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|---|---|---|
| `frontend/src/app/admin/exams/questions/page.tsx` | 수정 | 시험 유형 필터·컬럼, 사용 시험(특정 시험명 선택)·사용 여부(사용중/미사용) 필터·컬럼(시험명 배지) 추가. 조회조건 세션 상태(`SavedSearchState`)·컬럼 리사이즈 키(v2→v3, 컬럼 9개로 확장)도 함께 갱신. 문항 전체 로드를 고정 size 500 → `totalPages` 순회로 변경(총 문항이 500을 넘으면 뒤 페이지가 누락되던 버그 수정) |
| `frontend/src/types/index.ts` | 수정 | `QuestionSummary`에 `usedInExams?: string[]` 필드 추가 |

### 검증

- `npx tsc --noEmit` 통과.
- 브라우저 e2e: 시험 유형=리눅스마스터 1급 필터 → 105건(DB 실제 건수와 일치), 여기에 사용 여부=미사용 추가 → 5건(회차 정보 없어 시험 생성에서 제외된 문항 수와 정확히 일치). 전체 문항 수 표시가 500→629로 정상화됨을 확인.
...
- **수정 개요**: 키워드 검색을 문항 제목 기준으로 바꿨으나(HIST-20260714-001), 검색 입력창 위 라벨은 여전히 "문항 내용"으로 남아 있어 실제 동작과 불일치했다. 라벨 텍스트를 "문항 내용" → "문항 제목"으로 변경했다(328행). 표 컬럼 헤더("문항 제목 / 내용")는 표시 설명이므로 그대로 유지.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/admin/exams/questions/page.tsx` | 수정 | 검색 입력창 `<label>` 텍스트 `문항 내용` → `문항 제목` |

### 복원 방법
이 ID(HIST-20260714-002)만으로 복원 시: 328행 라벨을 다시 `문항 내용`으로 되돌린다.
...
- **날짜**: 2026-07-14
- **수정 범위**: 관리자 프론트엔드 / 문항 관리 — 키워드 검색을 "문항 제목" 기준으로 변경
- **수정 개요**: 기존 키워드 검색은 제목(title) 또는 본문(content)을 모두 매칭했는데, 본문이 RichTextEditor로 저장된 HTML 원문이라 태그·마크업이 섞여 검색 결과가 불안정했다. 검색 조건을 제목만 매칭하도록 좁히고, 검색창 placeholder를 "문항 제목 검색"으로 변경했다. 표 컬럼 헤더("문항 제목 / 내용")는 표시(제목 없으면 본문 노출) 설명이므로 그대로 유지했고, 본문 미리보기(`stripHtml`)는 여전히 표에서 사용되므로 import를 유지했다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| src/app/admin/exams/questions/page.tsx | 수정 | 키워드 검색 조건을 제목만 매칭하도록 축소, 검색창 placeholder 변경 |

### 수정 상세

#### `src/app/admin/exams/questions/page.tsx`
- 변경 전: `filtered` 내부에서 `inTitle`과 `inContent`(HTML 원문 포함 `q.content`) 둘 중 하나라도 매칭되면 통과, placeholder `"검색어를 입력하세요"`
- 변경 후: `inTitle`만으로 필터링(`inContent` 및 관련 로직 제거), placeholder `"문항 제목 검색"`
- 이유: HTML 원문 기반 본문 검색이 불안정해 제목 기준 검색으로 좁힘
...
- **수정 개요**: 관리자 문항 목록에서 AI 커스텀 배지 표시 조건이 `examYear·examRound·questionNo 모두 없음`이어서, 정렬을 위해 AI 커스텀 문제에 문항번호를 입력하면 배지가 사라지고 출처 문항처럼 "N번"만 표시되는 문제를 수정했다. 판정 기준을 **연도·회차 없음**으로 좁혀 문항번호가 있어도 AI 커스텀으로 인식하고, 배지 옆에 문항번호("N번")를 함께 표시한다. 문항번호 입력 자체는 등록/수정 폼·백엔드 모두 이미 연도·회차와 무관하게 허용되고 있었고(resolveQuestionNo가 명시 번호를 그룹 없이 저장), 출처순 정렬(compareQuestionSourceOrder)도 이미 연도·회차 null 그룹 안에서 questionNo 오름차순을 적용하므로 이 표시 수정만으로 요구사항이 충족된다. 사용자 퀴즈 화면 배지(:415)는 원래 연도·회차 기준이라 수정 불필요.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/admin/exams/questions/page.tsx` | 수정 | 출처/배지 분기 조건에서 questionNo 제거(연도·회차 기준), AI 커스텀 배지 + 문항번호 병기 표시 |

### 검증
- `npx tsc --noEmit` 통과.

### 복원 방법
이 ID(HIST-20260710-002)만으로 복원 시 분기 조건을 `(examYear != null || examRound != null || questionNo != null)`로 되돌리고 배지를 단독 `<span>`으로 되돌린다.
