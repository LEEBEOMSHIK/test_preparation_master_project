...
- **수정 개요**: 시험 회차 카드 목록을 회차 묶음 기준 5개씩 페이지로 나누어 표시.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/exam-info/page.tsx` | 수정 | 회차 묶음 단위 페이지 분할, 공용 Pagination, 목록 상단 이동 및 페이지 범위 보정 |
| `frontend/src/app/user/exam-info/page.test.tsx` | 추가 | 그룹 보존·페이지 이동·유형 변경·관심 저장 후 축소·별도 직접 접수 영역 검증 |

### 수정 상세

- 변경 전: 모든 시험 회차 카드가 한 화면에 표시되어 목록이 길어졌다.
- 변경 후: 기존 `sessionGroups` 생성 후 5개씩 나누며 같은 회차의 1차·2차는 같은 카드 안에 유지한다. 이전·번호·다음 이동은 공용 Pagination을 사용한다.
- 유형 변경 또는 관심 설정 저장 시 첫 페이지로 돌아간다. 관심 설정 저장 시 전체 유형으로 복귀하여 사라진 유형을 계속 선택하지 않도록 한다. 결과 수가 줄면 현재 페이지 범위를 보정한다.
- 페이지 이동 시 목록 상단으로 부드럽게 이동하고 모바일 고정 헤더를 고려한 scroll margin을 둔다. 직접 등록한 시험 접수 영역은 페이지 분할에서 제외한다.
- 이유: 목록 길이를 줄이면서 회차 관계와 별도 접수 정보 접근성을 유지한다.

### 검증

- RED: 미구현 상태에서 첫 페이지에 여섯 번째 회차가 노출되고 다음 페이지 버튼이 없어 3건 실패를 확인했다.
- GREEN: `npx jest --watch=false --runInBand src/app/user/exam-info/page.test.tsx` — 1 suite, 3 tests 통과.

### 복원 방법

`UserExamInfo_Modified.md`의 `HIST-20260909-001` 변경을 역적용하여 페이지 상태·분할·Pagination·스크롤 처리를 제거하고 기존 `sessionGroups.map` 전체 렌더링으로 복원한다.
...
- **날짜**: 2026-08-05
- **수정 범위**: 사용자 프론트엔드 / 시험 정보(`/user/exam-info`) — 시험 유형별 색상 일관화 및 다크 모드 필터 탭 대비 개선
- **수정 개요**: 시험명 문자 코드 합을 팔레트 인덱스로 사용하던 방식 때문에 서로 관련 없는 관심 시험이 같은 색으로 표시되던 문제를 수정했다. 관심 시험과 조회된 시험 유형을 합친 화면 표시 순서에 따라 12개 테마를 중복 없이 배정하고, 헤더 관심 시험 배지·유형 필터 탭·시험 카드 유형 배지가 동일한 시험 유형에 같은 색을 사용하도록 통합했다. 필터 탭에는 시험별 light/dark 활성·비활성·hover 배경, 테두리, 텍스트 색을 명시해 다크 모드에서도 상태 변화가 분명히 보이게 했다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/exam-info/page.tsx` | 수정 | 12개 `ExamTypeTheme` 정적 팔레트 정의, `allTypes` 순서 기반 `useMemo` 테마 `Map` 구성, 헤더·필터·카드 배지 공통 테마 적용, 다크 모드 필터 활성·hover 대비 보강 |
| `docs/agent-handoff/CURRENT.md` | 수정 | 구현 전 및 구현 완료 후 검증 시작 전 작업 상태·주의사항 기록 |

### 수정 상세

#### 시험 유형 색상 배정

- **변경 전**: `examTypeColor(name)`이 시험명 각 문자의 `charCode` 합을 팔레트 길이로 나눈 나머지를 사용해 색을 정했다. 해시 충돌로 서로 다른 시험 유형이 같은 색을 받을 수 있었고, 헤더/카드 배지만 이 함수를 사용했다.
- **변경 후**: `userInterests`와 `itemTypes`를 합친 `allTypes`의 안정적인 표시 순서로 `Map<string, ExamTypeTheme>`을 `useMemo`에서 생성한다. 12개 팔레트 범위에서는 각 유형이 서로 다른 테마를 받고, 유형이 팔레트보다 많을 때만 순환한다. 헤더 관심 시험 배지, 유형 필터 탭, 시험 카드 유형 배지는 모두 같은 Map을 조회한다.
- **이유**: 시험명 내용에 우연히 의존하는 해시 충돌을 없애고, 화면 안에서 시험 유형과 색상의 관계를 직관적이고 일관되게 유지하기 위해서다.

#### 다크 모드 필터 탭 상태

- **변경 전**: 모든 활성 탭이 `bg-indigo-600 text-white`, 비활성 탭이 `bg-gray-100 text-gray-600 hover:bg-gray-200`였고 다크 모드 전용 색상과 테두리가 없어 배경 및 hover 상태 구분이 약했다.
- **변경 후**: 시험 유형마다 활성/비활성 탭의 light/dark 배경·테두리·텍스트·hover 클래스를 완전한 문자열 상수로 선언했다. 활성 탭은 진한 시험별 색, 비활성 탭은 옅은 시험별 색과 명시적 테두리를 사용하며 다크 모드 hover에서는 배경과 테두리가 함께 밝아진다. `전체` 탭은 인디고 활성/회색 비활성 테마를 별도로 유지한다.
- **이유**: Tailwind 정적 스캔 누락 없이 모든 상태 클래스를 생성하고, 다크 모드에서도 기본 상태와 hover 상태의 대비를 확실하게 구분하기 위해서다.

### 복원 방법

`frontend/src/app/user/exam-info/page.tsx`에서 `ExamTypeTheme`, `EXAM_TYPE_THEMES`, `DEFAULT_EXAM_TYPE_THEME`, `EMPTY_EXAM_TYPES`, `examTypeThemeMap`을 제거하고 기존 `PALETTE` 및 시험명 문자 코드 합 기반 `examTypeColor(name)` 함수를 복원한다. 헤더와 카드 배지의 테마 조회를 `examTypeColor(...)`로 되돌리고, 유형 필터 탭을 공통 인디고 활성/회색 비활성 클래스 조건식으로 복원한다.

### 검증

- `npx tsc --noEmit` 통과(exit 0).
- Next.js 개발 서버가 `/user/exam-info`를 2.7초에 961 modules로 정상 컴파일했다.
- `GET http://localhost:3000/user/exam-info` 응답 200 확인.
- `git diff --check` 통과.
...
- **날짜**: 2026-08-03
- **수정 범위**: 사용자 프론트엔드 / 시험 정보(`/user/exam-info`) — 같은 회차 1차/2차 카드 그룹핑
- **수정 개요**: 리눅스마스터처럼 같은 회차 안에 1차/2차가 나뉜 시험은 완전히 별개의 카드 두 장으로 떨어져 있어 "같은 회차의 두 세션"이라는 관계를 한눈에 파악하기 어렵다는 피드백에 따라, 제목 끝의 "N차"를 기준으로 같은 회차 항목을 하나의 카드로 묶고 그 안에 1차/2차를 나란히 배치했다. 1차/2차 구분이 없는 시험(SQLD·정보처리기사)은 그룹 크기가 1이 되어 기존과 동일한 단일 카드로 렌더링된다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/exam-info/page.tsx` | 수정 | `SESSION_SUFFIX_RE`(제목 끝 "N차" 매칭) 신규 상수, `sessionGroups`(useMemo) 신규 — `displayed`를 `examType::N차 제외 제목` 키로 그룹핑. 카드 렌더링을 `renderSessionBody(item)` 로컬 함수로 추출(설명·접수/시험/발표 3박스·내 접수 정보 미니 섹션)해, 그룹 크기 1이면 기존과 동일한 단일 카드, 2 이상이면 그룹 헤더(시험유형 배지+회차 제목+공식홈페이지) 아래 세션별 서브패널(1차/2차 배지 + `renderSessionBody`)을 `sm:grid-cols-2`로 배치 |

### 검증

- `npx tsc --noEmit` 통과.
- 브라우저 확인: 리눅스마스터 1급·2급 각각 회차별(2601·2602 등) 카드 하나에 1차/2차가 나란히 표시되고 각 세션의 접수/시험/발표 상태·"내 접수 정보 입력"이 세션별로 독립적으로 동작함을 확인. 정보처리기사 실기(1차/2차 없음)는 기존과 동일한 단일 카드 레이아웃 유지 확인.
...
- **날짜**: 2026-08-03
- **수정 범위**: 사용자 프론트엔드 / 시험 정보(`/user/exam-info`) — 관심 시험 필터 탭 누락 버그 수정 + 데이터 보강
- **수정 개요**: 관심 시험에 "리눅스마스터 2급"을 추가해도 시험정보 화면의 필터 탭에 아예 나타나지 않는 문제가 있었다(사용자 리포트). 원인은 두 가지가 겹쳐 있었다 — (1) `exam_info`에 리눅스마스터 2급 데이터가 한 건도 없었고(f55fea5 마이그에서 domain_slave만 추가하고 exam_info 시드는 누락), (2) 필터 탭 목록(`allTypes`)이 서버가 반환한 `items`(이미 관심 시험으로 필터링된 결과)에서만 뽑혀서, 데이터가 0건인 관심 유형은 탭 자체가 생성되지 않았다. 사용자 피드백("데이터가 없어서 안 보이는 거라면 탭에서 아예 안 보이는 게 아니라 '시험 정보가 없다'고 표시하는 게 낫다")에 따라 탭 목록을 관심 시험 유형 기준으로 항상 생성하도록 고쳐, 향후 같은 상황(관심 유형은 있는데 데이터가 아직 없음)에서도 탭은 보이고 기존 빈 상태 메시지("표시할 시험 정보가 없습니다")로 안내되게 했다. 근본 원인인 exam_info 데이터 자체도 ihd.or.kr 공식 2026년 일정으로 8건(4회차 × 1차/2차) 보강했다 — 리눅스마스터 1급·SQLD와 동일하게 `DataInitializer`에 idempotent 시더 메서드를 추가하는 방식(→ `docs/history/back/adm/AdminExamInfo_Modified.md` HIST-20260803-001 참고).

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/exam-info/page.tsx` | 수정 | `allTypes` 산출 로직을 `items`(서버 반환 결과) 단독 기준 → `userInterests`(관심 시험 유형, 있으면 우선) ∪ `items`의 examType 기준으로 변경 |
| `docs/sql/tpmp_content_data.sql` | 수정 | `DataInitializer`가 생성한 리눅스마스터 2급 exam_info 8건을 exam_info 섹션 끝에 추가(기존 11건 행은 변경 없음) — 백엔드 미기동 상태에서 콘텐츠 덤프만 로드하는 로컬도 커버 |

### 검증

- `npx tsc --noEmit` 통과.
- 브라우저 e2e(사용자 계정, 관심 시험에 리눅스마스터 2급 포함 상태): 시험정보 화면 필터 탭에 "리눅스마스터 2급"이 정상 노출, 클릭 시 "리눅스마스터 2급 2026년 1회(2601) 1차" 카드(접수기간 2026.01.26~02.04, 시험일정 2026.01.27~02.05 범위 형식 포함)가 정상 렌더링됨을 확인.
- 임시 검증 DB로 베이스라인+콘텐츠 덤프 전체 재현 — ERROR 0건, exam_info 리눅스마스터 2급 8건 반영 확인 후 삭제(원본 `tpmp` DB에는 영향 없음).
