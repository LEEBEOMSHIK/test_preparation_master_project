...
- **날짜**: 2026-09-09
- **수정 범위**: 사용자 프론트엔드 / 내 개념노트·공개 탐색
- **수정 개요**: 내 노트 전체 서버 제목 검색과 목록 페이지 경계·조회 오류 처리를 개선한다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/concepts/page.tsx` | 수정 | 서버 검색, 삭제 재조회·페이지 보정, 경합·오류 대응, 공용 목록 표시 |
| `frontend/src/app/user/concepts/explore/page.tsx` | 수정 | 기본 5개 및 공용 목록 표시, 조회 경합·오류·범위 보정 |
| `frontend/src/app/user/concepts/page.test.tsx` | 추가 | 두 목록 검색·삭제·응답 경합·실패 재시도 회귀 테스트 |
| `frontend/src/services/conceptNoteService.ts` | 수정 | getMyNotes 선택 keyword 추가, 기존 두 인자 호출 유지 |

### 수정 상세

- 변경 전: 내 노트는 현재 페이지 제목만 로컬 검색하고 검색 중 페이지를 숨김. 삭제 시 화면 배열·건수만 차감해 빈 마지막 페이지 발생. 두 목록 조회 실패가 처리되지 않고 이전 응답이 새 결과를 덮을 수 있었음.
- 변경 후: 내 노트 API keyword로 전체 소유 노트를 검색하고 기본 5개, ListPagination으로 검색 중에도 이동·건수·크기 제공. 검색·크기 변경은 첫 페이지, 삭제 후 서버 재조회 및 유효 페이지 보정. effect 정리로 오래된 응답 무시, 오류와 재시도 표시. 고정 목록 래퍼로 이동 시 상단 스크롤 지원.
- 이유: 전체 검색 정확성 및 마지막 페이지 삭제·빠른 검색·네트워크 오류의 일관성 보장.
- 검증: Jest 6건 RED 확인 후 6건 통과. 백엔드 검증은 통합 단계에서 별도 실행.

### 복원 방법

이 파일의 `HIST-20260909-001` 변경만 되돌린다. 페이지 기본 10개·기존 페이지 표시 및 로컬 검색·삭제 반영을 복원하고 서비스 선택 keyword 인자를 제거한다. 공용 ListPagination은 다른 화면도 사용하므로 제거하지 않는다.
