...
- **날짜**: 2026-09-09
- **수정 범위**: 사용자 프론트엔드 / 로그인·회원가입 이용 연령 안내
- **수정 개요**: 확정된 만 14세 이상 이용 정책을 인증 진입 화면에 안내한다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/login/page.tsx` | 수정 | 공통 정책 `LEGAL_INFO.minimumAge`로 이용 연령 표시 |
| `frontend/src/app/auth/signup/page.tsx` | 수정 | 동일 정책으로 회원가입 전 이용 연령 표시 |
| `frontend/src/app/user/login/page.test.tsx` | 수정 | 만 14세 이상·미만 이용 제한 안내 노출 검사 |

### 수정 상세

- 변경 전: 로그인·회원가입의 개인정보 안내에 이용 가능한 연령이 없었음.
- 변경 후: `LEGAL_INFO.minimumAge`가 설정된 경우 해당 연령 이상 이용 가능 및 미만 이용 불가를 명시한다. 생년월일 수집·연령 인증·서버 차단·동의 체크박스는 추가하지 않는다.
- 이유: 사용자 확정 운영 정책을 로그인 전에 알리고 공개 문서와 동일한 정책 값을 사용한다.

### 복원 방법

`Login_Modified.md`의 `HIST-20260909-002` 기준으로 두 화면의 `LEGAL_INFO` import와 연령 안내 조건부 문단, 테스트의 연령 안내 assertion만 제거한다. 기존 개인정보 안내는 유지한다.
...
- **날짜**: 2026-09-09
- **수정 범위**: 사용자 프론트엔드 / 로그인·회원가입·공개 안내 진입점
- **수정 개요**: 로그인 전 개인정보 이용 안내와 공개 문서 진입점을 제공한다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/login/page.tsx` | 수정 | Google 계정 생성·수집 항목·메일함 권한 미요청 안내 및 공개 문서 링크 |
| `frontend/src/app/user/login/page.test.tsx` | 추가 | 안내·공개 경로 노출 및 기존 OAuth 실패 안내 회귀 검사 |
| `frontend/src/app/auth/signup/page.tsx` | 수정 | 기존 회원가입 수집 항목 안내 및 공개 문서 링크 |
| `frontend/src/app/page.tsx` | 수정 | 중복 테스트 바로가기를 공개 문서 링크로 대체 |
| `frontend/src/components/layout/UserLayoutShell.tsx` | 수정 | 모바일 하단 메뉴 위로 스크롤 가능한 공개 문서 footer |

### 수정 상세

- 변경 전: 로그인·회원가입 전에 개인정보 수집 항목이나 공개 문서로 이동할 경로가 없었음.
- 변경 후: Google 로그인 주변에 최초 계정 생성과 이메일·이름·Google 식별자 이용을 안내하고 Gmail 메일함 접근 권한은 요청하지 않음을 명시. 회원가입에는 이름·이메일·비밀번호 수집과 해시 저장을 안내. 홈·사용자 화면에서도 `LegalLinks`로 개인정보처리방침·이용약관·권리행사 안내에 접근 가능.
- 이유: 사용자가 인증 전에 데이터 이용 내용을 확인하고 이후에도 권리행사 안내에 접근하도록 함. 동의 자동 간주나 체크박스를 추가하지 않으며, 기존 로그인 및 회원가입 동작은 유지.

### 복원 방법

`Login_Modified.md`의 `HIST-20260909-001` 기준으로 위 파일의 안내·`LegalLinks` import 및 footer만 제거하고 홈 중복 테스트 링크를 복원한다. 테스트 파일은 해당 변경과 함께 제거한다. 공용 `LegalLinks`는 공개 페이지에서도 사용하므로 삭제하지 않는다.
