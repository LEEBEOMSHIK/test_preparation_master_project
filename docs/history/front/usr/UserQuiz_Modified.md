...
- **날짜**: 2026-07-16
- **수정 범위**: 사용자 프론트엔드 / 데일리 퀴즈 풀이 — 문항 제목(문항 관리 제목) 노출로 인한 정답 유추 방지
- **수정 개요**: 데일리 퀴즈 풀이 화면(일반 풀이 + 북마크 재풀이 모드 공용 카드)에서 문항 관리 제목(`q.title`)을 그대로 노출해 일부 문항에서 정답이 유추되는 문제가 있었다. 문제 카드 상단의 제목 헤더 블록을 제거해 풀이 중에는 제목이 보이지 않도록 했다. 복습/결과 화면(북마크 목록, `ExamResultDisplay`, `QuestionDetailModal`, 개념노트)은 대상에서 제외해 그대로 제목을 표시한다. 정식 시험 풀이(`app/exam/[id]/page.tsx`)는 애초에 `ExaminationQuestionView`에 title 필드가 없어 영향 없음. 파일 내 다른 위치(781/799행 부근)의 `title=` 속성은 버튼 HTML 툴팁으로 이번 변경과 무관.
- **관련**: [[HIST-20260716-001]] (같은 파일, 같은 날짜 선행 변경 — seenIds 영속화)

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/quiz/[categoryId]/page.tsx` | 수정 | 문항 관리 제목 헤더 블록(`q.title` 렌더) 삭제 |
...
```tsx
        {/* 발문(지시문) — 문항 내용 위에 강조 표시 */}
```
  (제목 헤더 블록 전체를 삭제하고, 바로 위 카테고리 배지 블록과 바로 아래 발문 블록 사이의 JSX 구조는 그대로 유지)
- 이유: 문항 제목이 정답을 직접적으로 암시하는 사례가 있어, 풀이 중에는 숨기고 복습·결과 화면에서만 노출되도록 범위를 제한

### 복원 방법
이 ID(HIST-20260716-002)만으로 복원 시 위 "수정 상세"의 "변경 전" 내용을 각 파일에 적용한다.
