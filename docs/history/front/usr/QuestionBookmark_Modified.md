...
- **날짜**: 2026-09-09
- **수정 범위**: 사용자 프론트엔드 / 복습 표시 목록 페이지네이션
- **수정 개요**: 배열로 조회한 복습 표시 문항을 기본 5개씩 나누고, 전체 건수·현재 범위·페이지 크기 선택·목록 상단 이동을 제공했다. 마지막 페이지의 유일한 문항을 제거하면 남아 있는 유효 페이지로 자동 보정한다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/bookmarks/page.tsx` | 수정 | 5개 기본 배열 페이지네이션, 크기 변경 시 첫 페이지 복귀, 삭제 후 페이지 보정 적용 |
| `frontend/src/app/user/bookmarks/page.test.tsx` | 추가 | 6개 경계와 마지막 페이지 문항 삭제 회귀 테스트 |
| `frontend/src/components/ui/ListPagination.tsx` | 추가 | 범위·건수·크기·축약 페이지 번호·상단 이동 공용 UI |
...
#### `frontend/src/app/user/bookmarks/page.tsx`
- 변경 전: 조회된 북마크 전체를 한 번에 렌더하고 삭제 후 현재 표시 범위를 보정하지 않았다.
- 변경 후: `page`와 `pageSize`로 현재 배열을 잘라 렌더하고, 크기 변경은 0페이지로 초기화하며 목록 감소 시 마지막 유효 페이지 이내로 보정한다.
- 이유: 긴 목록의 탐색성을 높이고 마지막 항목 삭제 후 빈 페이지에 머무는 문제를 막기 위해서다. 조회 실패는 빈 목록 대신 오류와 재시도를 표시한다.

### 복원 방법

이 ID(`QuestionBookmark_Modified.md` 기준 HIST-20260909-001)로 복원 시 북마크 페이지의 `ListPagination` 상태·배열 슬라이스·목록 래퍼 ID를 제거하고 전체 `bookmarks` 렌더 방식으로 되돌린다. 공용 컴포넌트는 다른 메뉴에서도 사용하므로 함께 제거하지 않는다.
