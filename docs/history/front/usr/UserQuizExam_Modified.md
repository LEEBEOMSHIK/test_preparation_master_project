...
- **수정 개요**: ① 퀴즈 카테고리 선택 후 문항 없으면 `router.back()` 대신 퀴즈 목록(`/user/quiz`)으로 이동, ② 시험 응시 화면에서 해당 문제에 등록한 개념노트 표시 및 수정 지원

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/app/user/quiz/[categoryId]/page.tsx` | 수정 | 문항 없음 시 `router.back()` → `router.push('/user/quiz')` |
| `frontend/src/app/exam/[id]/page.tsx` | 수정 | 시험 로드 시 개념노트 일괄 조회, questionNotes 맵 관리, 기존 노트 표시·수정 지원 |

### 수정 상세

#### `user/quiz/[categoryId]/page.tsx`
- 변경 전: `router.back()` — 이전 페이지(카테고리 선택 이외일 수 있음)로 이동
- 변경 후: `router.push('/user/quiz')` — 데일리 퀴즈 목록으로 정확히 이동
- 이유: 문항 없는 카테고리 선택 시 의도한 목록 화면으로 복귀

#### `exam/[id]/page.tsx`
- 변경 전: 개념노트 작성(신규 생성)만 가능, 기존 노트 조회 불가
- 변경 후:
  - 시험 로드 후 `getMyNotes(0, 500)` 호출 → `questionId` 기준으로 `questionNotes` 맵 구성
  - "메모" 버튼이 인디고 색상으로 강조되면 기존 노트 있음을 표시("노트"로 라벨 변경)
  - 노트 모달 열 때 기존 노트 있으면 제목·내용 불러와 수정 모드로 진입
  - 저장 시 기존 노트는 `update`, 신규는 `create` 호출
  - 모달 헤더 "개념노트 작성" / "개념노트 수정" 전환
- 이유: 시험 화면에서 본인이 등록한 개념노트를 확인하고 수정할 수 있도록 개선

### 복원 방법

HIST-20260422-003 복원 시:
- `quiz/[categoryId]/page.tsx`: `router.push('/user/quiz')` → `router.back()` 으로 되돌림
- `exam/[id]/page.tsx`: `questionNotes`, `noteId` 상태 및 관련 로직 제거, 기존 단순 create 방식으로 복원
