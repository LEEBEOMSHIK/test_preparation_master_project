...
- **날짜**: 2026-09-09
- **수정 범위**: 사용자 프론트엔드 / 데일리 퀴즈 카테고리 아이콘
- **수정 개요**: 모든 카테고리에 같던 전구 SVG를 실제 마스터 코드와 카테고리 이름의 의미에 따른 코드·DB·프로세서·네트워크·보안·소프트웨어 설계·웹·실기·필기·리눅스 SVG로 구분하고, 알 수 없는 이름에는 기존 전구 형태의 기본 아이콘을 제공했다.

### 수정 파일 목록

| 파일 경로 | 수정 유형 | 설명 |
|-----------|-----------|------|
| `frontend/src/components/ui/QuizCategoryIcon.tsx` | 추가 | ID·배열 순서에 의존하지 않는 의미 기반 카테고리 SVG 분기 |
| `frontend/src/app/user/quiz/page.tsx` | 수정 | 카테고리 카드에서 새 공용 아이콘 사용 |
| `frontend/src/app/user/quiz/page.test.tsx` | 추가 | 실제 시드 이름별 아이콘과 기존 코드 언어 선택 모달 회귀 테스트 |
| `AGENTS.md` | 수정 | 새 공용 UI 사용 표 등록 |

### 수정 상세

#### `frontend/src/components/ui/QuizCategoryIcon.tsx`
- 변경 전: 모든 카테고리 카드가 동일한 전구 SVG를 직접 렌더했다.
- 변경 후: `EXAM_TYPE` 마스터 코드와 운영체제·SQL·프로그래밍 언어·네트워크·정보보안·소프트웨어공학·관계형 DB 이론·웹 기술·시험명 문자열을 정규화해 의미별 SVG를 선택하며, 미등록 이름은 기본 전구로 안전하게 표시한다.
- 이유: 카테고리를 시각적으로 빠르게 구분하되 변경 가능한 데이터베이스 ID나 표시 순서에는 의존하지 않기 위해서다.

#### `frontend/src/app/user/quiz/page.tsx`
- 변경 전: 슬레이브 카드 내부에 전구 SVG가 인라인으로 중복됐다.
- 변경 후: `QuizCategoryIcon`에 `master.code`와 `slave.name`을 전달한다. 클릭 처리와 `CodeLanguageModal` 상태·이동 흐름은 변경하지 않았다.
- 이유: 표시만 개선하면서 기존 퀴즈 선택 동작을 보존하기 위해서다.

### 복원 방법

이 ID(`DailyQuiz_Modified.md` 기준 HIST-20260909-001)로 복원 시 카테고리 카드의 `QuizCategoryIcon` 호출을 기존 전구 SVG로 되돌리고 해당 공용 컴포넌트와 테스트를 제거한다.
