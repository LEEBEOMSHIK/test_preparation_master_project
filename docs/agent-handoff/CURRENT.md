...
- **데이터 작업**(커밋 대상 아님, 운영 DB): AI 커스텀 문항 20개 등록 — 관계형 DB 이론(31) 113~117, 소프트웨어공학(30) 118~127(테스트 기법·UML), 네트워크(4) 128~132(서브넷 계산). 시험 유형 모두 "정보처리기사 실기"(7)·연도/회차 null. 정답 없는 중복 문항 111 소프트 삭제. 전 문항 등록 직후 채점 API로 정답/오답 케이스 검증 완료.
...
- `43099d1 [FE|BE] feat: 데일리 퀴즈 세션 내 중복 출제 방지 및 라운드 안내 문구` — main 푸시 완료
