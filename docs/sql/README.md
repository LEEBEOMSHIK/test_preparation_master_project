...
2. **날짜순 델타 마이그레이션 적용** — `docs/db-migration/`의 `YYYYMMDD_*.sql` 파일을 파일명 오름차순으로 적용합니다. 베이스라인 이후 추가된 스키마를 반영합니다.
...
> ### ❗ 델타 마이그레이션은 베이스라인 이후 날짜순으로 적용하세요
>
> `20260701_01`부터 최신 파일까지의 델타는 베이스라인에 반영되지 않은 후속 변경을 포함합니다. 신규 DB도 베이스라인 다음에 파일명 오름차순으로 적용해야 최신 스키마가 됩니다.
>
> 신규 DB에 필요한 스키마 적용 순서는 다음과 같습니다.
> - **기준 스키마** → 베이스라인 파일 1개
> - **후속 변경** → 날짜순 델타 마이그레이션 전체
> - **데이터** → `tpmp_content_data.sql`
>
> 델타 파일은 변경 이력인 동시에 적용 대상입니다. 각 파일은 적용 전에 목적·전제 조건을 확인하세요.
...
| 다른 컴퓨터에 **일부 스키마·데이터가 이미 있음** | 1 → 2 → 4 (베이스라인과 델타로 없는 테이블·컬럼을 채워 넣고, 덤프가 없는 콘텐츠만 채웁니다. 적용 전 각 델타의 전제 조건을 확인합니다.) |
| 계속 개발해온 최신 로컬 | 새 델타가 있을 때 2만 적용 (없으면 아무것도 안 해도 됨) |

베이스라인·콘텐츠 덤프는 **재실행해도 안전**합니다. 델타 마이그레이션은 파일별 전제 조건을 확인한 뒤 적용하세요.
...
### 델타 이력 (베이스라인 이후 파일명 순서로 적용)

아래 델타는 베이스라인 이후의 변경 이력입니다. 신규·기존 환경 모두 베이스라인 다음에 파일명 오름차순으로 적용해 최신 스키마를 맞춥니다.
...
| 36 | `20260826_01_create_patch_notes.sql` | 관리자 작성 패치노트 테이블(`patch_notes`)과 관리자·사용자 목록 조회 인덱스 생성 |
...
### 0-1) 날짜순 델타 마이그레이션 적용

베이스라인 적용 후 `docs/db-migration/`의 `YYYYMMDD_*.sql` 파일을 파일명 오름차순으로 적용합니다. 새로 추가된 테이블·컬럼·인덱스는 이 단계에서 반영됩니다.
