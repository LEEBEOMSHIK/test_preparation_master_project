...
2. **문의·요청 워크플로 델타 적용** — `docs/db-migration/20260828_01_extend_inquiry_workflow.sql`을 실행합니다. 빈 DB에서는 스키마만 만들고 문의 도메인 시드는 보류하므로 기존 고정 도메인 ID를 선점하지 않습니다. **`ddl-auto=validate`인 프로덕션과 신규 DB 모두 최초 백엔드 기동 전에 필수**입니다.
...
>
> 기존 35개 델타 파일은 **변경 이력 추적용으로 보존**하며, 실행 대상이 아닙니다. 단, 문의·요청 워크플로를 사용하는 모든 환경은 아래 `20260828_01_extend_inquiry_workflow.sql`을 베이스라인 적용 후 추가 실행해야 합니다. 이 파일은 기존 스키마·데이터 이관과 신규 워크플로 테이블 생성을 함께 처리하며 재실행해도 안전합니다.
...
베이스라인·문의 델타·콘텐츠 덤프 모두 **재실행해도 안전**하므로, 환경 상태가 확실하지 않으면 1 → 2 → 3 → 4 → 5 → 6 전체 순서를 따릅니다.
...
| 36 | `20260828_01_extend_inquiry_workflow.sql` | 문의·요청 접수 유형·상태·발생 영역·대화 메시지·첨부 업로더·메일 알림 이력을 추가하고 기존 문의와 답변을 이관. **신규·기존 DB 모두 베이스라인 적용 후 실행 필요** |
...
최초 백엔드 기동 전 한 번, 콘텐츠 데이터 로드 후 한 번 실행합니다. 두 번째 실행에서 문의 도메인 값을 기존 콘텐츠 ID 뒤에 최종 구성합니다.
...
- 베이스라인 스키마를 적용하지 않은 채 콘텐츠 데이터를 로드하면 `examinations.exam_year` 등 없는 컬럼/테이블(`support_settings`) 관련 오류로 즉시 실패합니다. 반드시 이 문서 최상단의 6단계 로드 순서를 지키세요.
