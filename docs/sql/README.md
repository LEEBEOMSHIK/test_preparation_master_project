...
> 마지막 갱신: 2026-08-30 (패치노트·문의·요청 워크플로 델타의 신규·기존 환경 적용 순서 통합)

## ⚠️ 로드 순서 (문의·요청 워크플로는 반드시 6단계로 진행)
...
6. **백엔드 재기동** — 최종 스키마와 도메인 구성을 검증하고 애플리케이션 시드를 멱등 보완합니다.

> ### ❗ 과거 델타 35개는 신규 DB에 적용하지 마세요
>
...
| 내 환경 | 해야 할 것 |
|---------|-----------|
| 저장소를 새로 클론 / 빈 DB | 1 → 2 → 3 → 4 → 5 → 6 전부 |
| 다른 컴퓨터에 **일부 스키마·데이터가 이미 있음** | 1 → 2 → 3 → 4 → 5 → 6 (멱등 실행으로 누락분만 보완) |
| 계속 개발해온 최신 로컬 | 2 → 5 → 6 (콘텐츠 덤프 누락분도 동기화하려면 4 포함) |

베이스라인·패치노트 델타·문의 델타·콘텐츠 덤프 모두 **재실행해도 안전**하므로, 환경 상태가 확실하지 않으면 1 → 2 → 3 → 4 → 5 → 6 전체 순서를 따릅니다.
...
### 델타 이력 (기존 35개는 보존용 — 신규 DB에서 실행하지 말 것)
...
docker cp docs/db-migration/20260826_01_create_patch_notes.sql tpmp-db:/tmp/patch-notes-delta.sql
...
위 0-1 단계에서 최초 백엔드 기동 전에 한 번 적용한 뒤, 콘텐츠 데이터 로드 후 아래 명령을 다시 실행합니다. 두 번째 실행에서 문의 도메인 값을 기존 콘텐츠 ID 뒤에 최종 구성합니다.
