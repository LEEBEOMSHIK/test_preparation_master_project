...
> 마지막 갱신: 2026-08-26 (신규 환경의 베이스라인 이후 날짜순 델타 마이그레이션 적용 절차와 patch_notes 스키마 반영)

## ⚠️ 로드 순서 (반드시 4단계로 진행)

이 프로젝트는 아직 Flyway/Liquibase를 쓰지 않고 `docs/db-migration/`에 수동 SQL 파일을 쌓아가는 방식입니다(`docs/db-guidelines.md` §10 참고).

1. **베이스라인 스키마 적용** — `docs/db-migration/00000000_00_baseline_schema.sql`을 실행합니다. 33개 기준 테이블을 만듭니다.
2. **베이스라인 이후 신규 델타 적용** — `20260826_01_create_patch_notes.sql`부터의 신규 파일만 파일명 오름차순으로 적용합니다. 베이스라인에 이미 반영된 과거 델타는 실행하지 않습니다.
...
> ### ❗ 과거 델타는 신규 DB에 적용하지 마세요
>
> `20260701_01` ~ `20260802_01`의 35개 파일은 **이미 베이스라인에 반영된 변경 이력**입니다. 이 중 일부는 데이터 보정·시드 데이터 전제를 가지므로, 빈 DB에서 실행하면 실패하거나 콘텐츠 덤프와 중복될 수 있습니다.
>
> 신규 DB에 필요한 스키마 적용 순서는 다음과 같습니다.
> - **기준 스키마** → 베이스라인 파일 1개
> - **후속 변경** → `20260826_01` 이후 신규 델타 마이그레이션만 날짜순 적용
> - **데이터** → `tpmp_content_data.sql`
>
> 과거 델타는 변경 이력 추적용으로 보존합니다. 신규 DB에는 베이스라인 이후 추가된 델타만 적용합니다.

### 상황별 필요한 단계

| 내 환경 | 해야 할 것 |
|---------|-----------|
| 저장소를 새로 클론 / 빈 DB | 1 → 2 → 3 → 4 전부 |
| 다른 컴퓨터에 **일부 스키마·데이터가 이미 있음** | 1 → 2 → 4 (베이스라인과 베이스라인 이후 신규 델타로 없는 테이블·컬럼을 채워 넣고, 덤프가 없는 콘텐츠만 채웁니다.) |
| 계속 개발해온 최신 로컬 | 새 베이스라인 이후 델타가 있을 때 2만 적용 (없으면 아무것도 안 해도 됨) |

베이스라인·콘텐츠 덤프는 **재실행해도 안전**합니다. 베이스라인 이후 신규 델타는 파일별 전제 조건을 확인한 뒤 적용하세요.
...
### 베이스라인 (신규 DB 초기 적용 파일)

| 파일 | 요약 |
|------|------|
| `00000000_00_baseline_schema.sql` | **33개 기준 테이블 정의** — 2026-08-02 기준 원본 로컬 DB의 `pg_dump --schema-only` 결과. 재실행 안전하며, 이미 일부 스키마가 있는 DB에는 없는 테이블·컬럼·제약·인덱스만 채워 넣습니다. |

### 과거 델타 이력 (보존용 — 신규 DB에서 실행하지 말 것)

아래 35개는 베이스라인에 이미 반영된 변경 이력입니다. 신규 DB에서는 실행하지 않습니다.
...
| 35 | `20260802_01_create_linux_sqld_exams.sql` | 리눅스마스터 1급/2급·SQLD 기출문제(`question_bank`)에 대응하는 시험지(`exams`)·시험(`examinations`)·시험문항(`questions`) 신규 생성 — 스키마 변경 없음(데이터만) |

> 번호 30 다음이 32인 이유: `20260722_03`는 결번(작업 중 취소되어 파일명이 존재하지 않음)입니다. 위 표는 `20260802_01`까지의 과거 델타 목록이며, 이후 파일은 아래 `베이스라인 이후 신규 델타` 표에 추가합니다.

### 베이스라인 이후 신규 델타 (신규 DB 적용 대상)

아래 파일은 베이스라인에 포함되지 않았으므로, 신규·기존 환경 모두 베이스라인 적용 뒤 파일명 오름차순으로 적용합니다.
...
새 스키마 변경은 `{YYYYMMDD}_{순번}_{설명}.sql` 델타 파일로 만들고, `베이스라인 이후 신규 델타` 표에 한 줄 추가합니다. 베이스라인 파일은 손대지 않습니다. 델타가 다시 많이 쌓여 신규 DB 구축이 번거로워지면, 그때 새 날짜의 베이스라인으로 교체하고 이전 델타를 `docs/db-migration/archive/`로 옮깁니다.
...
베이스라인 적용 후 `20260826_01_create_patch_notes.sql`부터의 신규 델타만 파일명 오름차순으로 적용합니다. `20260802_01`까지의 과거 델타는 베이스라인에 이미 반영되어 있으므로 실행하지 않습니다.
...
- 베이스라인 스키마와 베이스라인 이후 신규 델타를 적용하지 않은 채 콘텐츠 데이터를 로드하면 `examinations.exam_year` 등 없는 컬럼/테이블(`support_settings`, `patch_notes`) 관련 오류가 발생할 수 있습니다. 반드시 이 문서 최상단의 "로드 순서" 4단계를 지키세요.
