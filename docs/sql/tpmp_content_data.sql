...
INSERT INTO public.questions (id, answer, code, content, explanation, language, options, question_type, seq, source_file, exam_id, category_id) VALUES (83, '4,2,3,5,1', '', '<p>다음은 데이터베이스 설계 절차를 순서대로 나타낸 것이다. 각 빈칸 ( ㄱ )~( ㅁ )에 들어갈 알맞은 용어를 [보기]에서 골라 순서대로 쓰시오.</p>
<p><strong>DB 설계 절차</strong></p>
<p>( ㄱ )<br>↓<br>( ㄴ )<br>↓<br>( ㄷ )<br>↓<br>( ㄹ )<br>↓<br>( ㅁ )</p>', '데이터베이스 설계 절차: 요구사항 분석 → 개념적 설계(E-R 모델) → 논리적 설계(릴레이션 스키마 변환·정규화) → 물리적 설계(저장 구조·인덱스) → 구현(DDL 작성·데이터 적재)', 'other', '["구현", "개념적 설계", "논리적 설계", "요구사항 분석", "물리적 설계"]', 'SHORT_ANSWER', 3, NULL, 5, 2) ON CONFLICT DO NOTHING;
...
INSERT INTO public.questions (id, answer, code, content, explanation, language, options, question_type, seq, source_file, exam_id, category_id) VALUES (86, '1. 정보 프레임(정보) / 2. 감독 프레임(감독) / 3. 비번호 프레임(비번호) / 4. 비동기 균형 모드(ABM) / 5. 비동기 응답 모드(ARM)', '', '<p>HDLC는 비트 중심의 데이터 링크 제어 프로토콜로, 프레임 단위로 데이터를 전송하며 흐름 제어 및 오류 복구 기능을 제공한다. 다음 설명에 알맞은 용어를 쓰시오.</p>
<ol>
...
INSERT INTO public.questions (id, answer, code, content, explanation, language, options, question_type, seq, source_file, exam_id, category_id) VALUES (90, '① CONSTRAINT / ② FOREIGN(FOREIGN KEY) / ③ TEAM_ID / ④ REFERENCES / ⑤ TEAM_ID2', '', '<p>다음 조건을 참고하여 괄호 ①~⑤에 들어갈 적절한 예약어 또는 칼럼명을 쓰시오.</p>
...
INSERT INTO public.questions (id, answer, code, content, explanation, language, options, question_type, seq, source_file, exam_id, category_id) VALUES (98, '2', 'SELECT COUNT(*)
FROM employee e
JOIN dept d ON e.dep_id = d.dept_id
WHERE d.budget > (
    SELECT AVG(budget) FROM dept
);', '<p>다음 두 테이블과 SQL을 참고하여 실행 결과로 출력되는 값을 쓰시오.</p>
...
<table>
<tr><th>dept_id</th><th>budget</th></tr>
<tr><td>10</td><td>100</td></tr>
<tr><td>20</td><td>300</td></tr>
<tr><td>30</td><td>200</td></tr>
</table>', '서브쿼리 AVG(budget) = (100+300+200)/3 = 200. budget > 200인 부서는 dept_id 20(300)뿐이고, employee에서 dep_id = 20인 행이 2개이므로 JOIN 결과는 2행 → COUNT(*) = 2', 'sql', '[]', 'SHORT_ANSWER', 18, NULL, 5, 2) ON CONFLICT DO NOTHING;
...
INSERT INTO public.questions (id, answer, code, content, explanation, language, options, question_type, seq, source_file, exam_id, category_id, disable_alternative_answer) VALUES (35, '① int a = 0 / ② a < m || b[a] < x / ③ b[a] < 0 / ④ b[a] = -b[a]; / ⑤ a++; / ⑥ return 1; / ⑦ ③ → ④ → ⑤ → ② → ⑥', '', '<p>문장(Statement) 커버리지 테스트를 수행하려고 한다. 아래 코드를 제어 흐름도 빈칸에 연결되도록 작성하고, 문장 커버리지 순서대로 작성하시오.</p><pre><code>int Main(int b[], int m, int x) {
    int a = 0;
    while (a &lt; m || b[a] &lt; x) {
        if (b[a] &lt; 0)
            b[a] = -b[a];
    ...
    return 1;
}</code></pre><p>1.( ① ) 2.( ② ) 3.( ③ ) 4.( ④ ) 5.( ⑤ ) 6.( ⑥ )</p><p>문장 커버리지 순서: 1 → 2 → ( ⑦ )</p>', '', 'other', '[]', 'SHORT_ANSWER', 15, NULL, 2, 30, true) ON CONFLICT DO NOTHING;
...
INSERT INTO public.question_bank (id, create_dt, create_uno, del_yn, modified_dt, modified_uno, use_yn, answer, code, content, exam_round, exam_year, explanation, language, options, question_type, title, category_id, exam_type_id, ai_difficulty, ai_domains, ai_keywords, ai_summary, scheduling_data, instruction, question_no, sql_data) VALUES (3, '2026-05-29 01:16:43.933619', 1, 'N', '2026-07-17 12:00:00', 1, 'Y', '4,2,3,5,1', NULL, '<p><strong>DB 설계 절차</strong></p>
<p>( ㄱ )<br>↓<br>( ㄴ )<br>↓<br>( ㄷ )<br>↓<br>( ㄹ )<br>↓<br>( ㅁ )</p>', 1, 2026, '데이터베이스 설계 절차: 요구사항 분석 → 개념적 설계(E-R 모델) → 논리적 설계(릴레이션 스키마 변환·정규화) → 물리적 설계(저장 구조·인덱스) → 구현(DDL 작성·데이터 적재)', 'other', '["구현", "개념적 설계", "논리적 설계", "요구사항 분석", "물리적 설계"]', 'SHORT_ANSWER', '2026년 1회 3번 — 데이터베이스 설계 절차', 2, 7, NULL, NULL, NULL, NULL, NULL, '다음은 데이터베이스 설계 절차를 순서대로 나타낸 것이다. 각 빈칸 ( ㄱ )~( ㅁ )에 들어갈 알맞은 용어를 [보기]에서 골라 순서대로 쓰시오.', 3, NULL) ON CONFLICT DO NOTHING;
...
INSERT INTO public.question_bank (id, create_dt, create_uno, del_yn, modified_dt, modified_uno, use_yn, answer, code, content, exam_round, exam_year, explanation, language, options, question_type, title, category_id, exam_type_id, ai_difficulty, ai_domains, ai_keywords, ai_summary, scheduling_data, instruction, question_no, sql_data) VALUES (18, '2026-05-29 01:16:43.940619', 1, 'N', '2026-07-17 12:00:00', 1, 'Y', '2', 'SELECT COUNT(*)
FROM employee e
JOIN dept d ON e.dep_id = d.dept_id
WHERE d.budget > (
    SELECT AVG(budget) FROM dept
...
<tr><td>20</td><td>300</td></tr>
<tr><td>30</td><td>200</td></tr>
</table>', 1, 2026, '서브쿼리 AVG(budget) = (100+300+200)/3 = 200. budget > 200인 부서는 dept_id 20(300)뿐이고, employee에서 dep_id = 20인 행이 2개이므로 JOIN 결과는 2행 → COUNT(*) = 2', 'sql', NULL, 'SHORT_ANSWER', '2026년 1회 18번 — SQL JOIN과 서브쿼리 결과', 2, 7, NULL, NULL, NULL, NULL, NULL, '다음 두 테이블과 SQL을 참고하여 실행 결과로 출력되는 값을 쓰시오.', 18, NULL) ON CONFLICT DO NOTHING;
...
INSERT INTO public.question_bank (id, create_dt, create_uno, del_yn, modified_dt, modified_uno, use_yn, answer, code, content, exam_round, exam_year, explanation, language, options, question_type, title, category_id, exam_type_id, ai_difficulty, ai_domains, ai_keywords, ai_summary, scheduling_data, instruction, question_no, sql_data) VALUES (6, '2026-05-29 01:16:43.937621', 1, 'N', '2026-07-08 16:09:13.890936', 1, 'Y', '1. 정보 프레임(정보) / 2. 감독 프레임(감독) / 3. 비번호 프레임(비번호) / 4. 비동기 균형 모드(ABM) / 5. 비동기 응답 모드(ARM)', NULL, '<p><br></p><ol><li>HDLC 프레임의 구성 단위로, 실제 사용자 데이터를 전송하는 프레임</li><li>데이터 링크의 흐름을 관리하고 오류 제어 및 통신 상태를 감시하는 프레임</li><li>순서 번호 없이 링크 설정, 해제, 모드 설정 등 제어 기능을 수행하는 프레임</li><li>두 국(Station)이 동등한 위치에서 서로 명령과 응답을 주고받는 모드</li><li>종국(Secondary)이 주국(Primary)의 허가 없이도 자발적으로 응답을 전송할 수 있는 모드</li></ol>', 1, 2026, NULL, 'other', NULL, 'SHORT_ANSWER', '2026년 1회 6번 — HDLC 프레임 및 모드', 4, 7, NULL, NULL, NULL, NULL, NULL, 'HDLC는 비트 중심의 데이터 링크 제어 프로토콜로, 프레임 단위로 데이터를 전송하며 흐름 제어 및 오류 복구 기능을 제공한다. 다음 설명에 알맞은 용어를 쓰시오.', 5, NULL) ON CONFLICT DO NOTHING;
